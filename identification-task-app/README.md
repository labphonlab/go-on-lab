...
参加者が日本語話者でない場合、`?lang=en` を付けると画面が英語になる。

```
index.html?cond=en_conversation&list=1&lang=en
```

リスト JSON に `"ui_lang": "en"` を書いておけば URL に付け忘れても切り替わる。

英語化されるのは**画面の枠**——フォーム項目、ボタン、進行中のメッセージ。
同意文と教示（`consentTitle` / `consentBody` / `instructionsBody`）は研究ごとに
内容が違うので訳語を用意していない。**英語で実施するなら設定パネルでこの 3 つを
英語にしてください。**設定した値は言語切り替えより優先されるので、上書きされる
心配はない。

訳語は `index.html` 内の `UI_EN` にある。文言を増やしたり画面を書き換えたら

```
python3 tools/extract_i18n.py          # 過不足を確認
python3 tools/extract_i18n.py --write  # 新しい要素に data-i18n を付ける
```

を実行する。`data-i18n` の付いていない日本語が残っていれば表示されてしまうので、
画面を触ったら必ず流すこと。

なお**ラジオボタンの `value`（`男性` など）は英語にしない**。これは記録される
回答コードそのもので、訳すと同じ研究の日本語群と英語群が同じ回答を別の値で
記録することになる。表示だけ英語になり、シートには一貫した値が入る。

## 動作確認（自動）

288 試行を手でクリックして確認するのは現実的でないので、jsdom 上で
実際の `index.html` を最後まで走らせるスクリプトを用意してある。

```
npm install                       # jsdom は devDependency
node identification-task-app/tools/e2e_gating.js <配布ディレクトリ> <条件> <リスト>
```

同意 → デモグラフィック → 教示 → ヘッドホン検査 → 練習 → 本試行 → 送信まで
通し、送信データの形（事例数・ゲート数・昇順・練習の除外・`isCorrect` の整合）
を検査する。オプション:

| オプション | 見るもの |
| --- | --- |
| `--speaker` | 逆位相の音を選ぶ＝スピーカー利用者が弾かれるか |
| `--offline` | 送信失敗時に 3 回リトライしてエラー報告が飛ぶか、参加者には成功画面が出るか |

**音に関することは何も分からない。**jsdom は音声をデコードしないので、刺激が
実際に鳴るか、270 ファイルのプリロードに何秒かかるか、逆位相音がスピーカーで
本当に打ち消されるかはブラウザで確かめること。
...
実験者は被験者にリンクを配布する前に以下を確認してください：

### 設定
- [ ] `?config=1` で設定パネルを開き、パラダイム・刺激リスト・回答ラベルが正しい
- [ ] `dataServerUrl` が**本格実験用 Apps Script** の URL を指している（デモ用ではない）
- [ ] `experimentName` が他の実験と被らない（例: `vot_p_b_2026spring`）
- [ ] Apps Script コード内の `RESEARCHER_EMAIL` に研究者のアドレスが設定済み
- [ ] 練習試行数・本試行数・休憩・ITI・ISI が研究設計に合っている

### 動作確認
- [ ] 自分のスマホ（できれば iPhone と Android 両方）で 1 回フル試行
- [ ] スプレッドシートに `participants` と `results_*` の 2 シート（パラダイム別）が作成された
- [ ] スプレッドシートの参加者行の各列が空欄なく入っている
- [ ] 全試行のデータが入っている（試行数 × 行数の確認）
- [ ] エラーメール通知が動く（送信失敗を意図的に起こしてテスト可: `dataServerUrl` を一時的に壊して試行）

### 配布
- [ ] 配布 URL から `?config=1` が外れている
- [ ] iPhone の消音スイッチ確認・自動ロック設定の案内文を被験者に同送
- [ ] 個人情報の取り扱いについて、研究倫理審査を通過済み

## 設定パネル（実験者用）

`?config=1` を URL に付けてアクセスすると、コードを編集せずに全設定を変更できます。

```
https://.../identification-task-app/?config=1
```

### パネルでできること
- **パラダイム選択**（Identification / AX / AXB / Rating）
- **刺激リスト**の表編集（行追加・削除）
- **試行構造**（反復数・練習・休憩・fixation/ITI/ISI）
- **回答ラベル**（paradigm 別）
- **同意・教示文言**の差し替え（HTML 可）
- **設定の JSON 書き出し / 読み込み**（バックアップ・配布用）
- **共有 URL コピー**（設定を URL ハッシュに埋め込む）

設定は localStorage に自動保存されます。設定変更後「保存して実験を開始」を押すと、
そのまま被験者向けフローに遷移できます。

派生実験を作る場合は、設定パネルでパラダイム・刺激・文言を編集して JSON で書き出し、
コードはそのままで運用してください。
