# 音声知覚 Identification タスク (授業デモ・テンプレート)

> **位置づけ**: このアプリは **授業デモ用**（pie / buy 連続体）と、**カスタム実験を組むためのテンプレート**を兼ねています。
> 販売を予定している研究グレードの刺激音セット（Klatt 完全合成・著作権クリーン）は別フォルダで提供します。
>  → 商用版の構成: `product/` および `pilot/jpn_kg_klatt/`
>  → 商用版のデータタグ: `pilot_*` または `release_*`（本デモの `class_demo_*` とは別系統）
...
| 用途 | 設定 |
|---|---|
| **授業デモ**（pie / buy 連続体） | DEFAULT のまま使用（experimentName: `class_demo_pie_buy_v1`） |
| **カスタム実験** | 設定パネル（`?config=1`）で responseOptions・刺激リストを変更 |

データはすべて Google Sheets に `experimentName` カラム付きで保存されるので、`class_demo_*` でフィルタすれば授業データだけ、`pilot_*` / `release_*` でフィルタすれば研究/商用データだけを抽出できます。

## 概要

- 1試行ごとに 1 つの音声刺激を提示する identification タスク
- 参加者は提示された音を聞き、回答ボタン（初期設定では `pie` / `buy`）から選択
