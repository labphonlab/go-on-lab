...
- 解析層は入力テキストの明らかな不備（誤字・文字化け・文の欠落等）を教育的に補完・修正・
  再構成してよいが、変更した場合は必ず変更前後と理由をreport.mdに記録し、人手承認を経て
  から納品する（音声が対応するセクションでは、書き起こしレベルの補正に留め、音声内容と
  異なる修正はしない — アラインメント・採点の不整合を防ぐため）
...
- [x] （代替対応）**FL用実頻度データ**: NGSL本家の代わりに`wordfreq`（Robyn Speer, Apache-2.0）
      ...
      （`report.md`に警告表示）
- [x] 入力テキストの補完・修正・再構成（`ClaudeClassifier`のみ。`original_text`/
      `revision_note`で全変更を`report.md`に記録し人手承認を必須化。音声対応セクションは
      書き起こしレベルの補正に制限）
