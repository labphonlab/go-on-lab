...
- content_type 5種類すべてに対応、AGENTS.mdが列挙する学習方法をすべて実装:
  - `vocabulary_list` → フラッシュカード（SM-2 SRS）、聞き取り選択問題（4択、L1加重ND高語を優先ディストラクタ化）
  - `dialogue` → ディクテーション、シャドーイング（3段階）、ロールプレイ（片側ミュート＋自己録音確認）
  - `grammar_note` → 意味理解→穴埋め→並べ替えの3段階ドリル
  - `reading_passage` → 音読フォロー＋理解度チェック
  - `pattern_drill` → モデル確認→産出練習→タイマー付き流暢性チェック
- 出力: 上記8学習方法を実装したNext.js静的PWA。UIは長時間の学習でも疲れにくい配色・
...
`ClaudeClassifier`（本番解析）は、OCR/PDF/docx変換由来の誤字・文字化け・文の途中切れなど
明らかな不備がある場合、学習項目として提示する前に教育的に適切な形へ補完・修正・再構成してよい。
ただし常に透明性を保つ:

- 変更した項目は必ず `original_text`（変更前）と `revision_note`（何をどう変えたか・理由）を
  中間JSONに残し、`report.md` の「入力テキストの補完・修正・再構成（要確認）」セクションに
  一覧で表示される。納品前に必ず目視で承認すること
- **音声が対応するセクションでは、書き起こしの誤字レベルの補正に留まる**よう指示している
  （実際の発話と異なる修正をすると、MFAアラインメントのタイムスタンプとディクテーションの
  採点が音声内容とずれてしまうため）。音声のないセクション（grammar_noteの例文等）では
  より踏み込んだ補完・再構成が許容される
- `HeuristicClassifier`（`--mock`）はパターンマッチのみで文章生成能力が無いため、
  このような修正は一切行わない（`original_text`/`revision_note`は常に空）
